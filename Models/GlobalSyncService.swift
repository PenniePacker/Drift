...
actor GlobalSyncService {
    ...
    func syncIfNeeded() async {
        ...
        let toSync = artists.filter { artist in
            guard let contribution = lastContribution(for: artist) else { return true }
            return artist.confirmedSessionCount > contribution.submittedSessionCount
        }

        for artist in toSync {
